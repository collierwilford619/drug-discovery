...
class BoltzWrapper:
    ...
    def preprocess_data_for_boltz(
        self, valid_molecules_by_uid: dict, score_dict: dict, final_block_hash: str
    ) -> None:
        ...
        for uid, valid_molecules in valid_molecules_by_uid.items():
            ...
            if self.subnet_config["num_molecules_boltz"] > 1:
                try:
                    ...
                    bt.logging.error(
                        f"Error computing Boltz subset entropy for UID={uid}: {e}"
                    )
        ...
        bt.logging.info(
            f"Writing {len(self.unique_molecules)} unique molecules to input directory"
        )
...
properties:
    ...
    def score_molecules_target(
        ...
        self.preprocess_data_for_boltz(
            valid_molecules_by_uid, score_dict, final_block_hash
        )
