...
def molecule_unique_for_protein_api(protein: str, molecule: str) -> bool:
    ...
    try:
        ...
        if response.status_code != 200:
            bt.logging.error(
                f"Failed to check molecule uniqueness: {response.status_code} {response.text}"
            )
            return True
...
def molecule_unique_for_protein_hf(protein: str, smiles: str) -> bool:
    ...
    try:
        cached_protein, cached_sha, inchikeys_set, last_check_time = (
        ...
        if cached_sha is None or (current_time - last_check_time > metadata_ttl):
            ...
            if cached_sha != current_sha:
                ...
                molecule_unique_for_protein_hf._CACHE = (
                    molecule_unique_for_protein_hf._CACHE[:3] + (last_check_time,)
                )
