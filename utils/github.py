...
def upload_file_to_github(filename: str, encoded_content: str):
    ...
    github_repo_path = os.environ.get(
        "GITHUB_REPO_PATH"
    )  # example: /data/results or ""

    if (
        not github_repo_name
        or not github_repo_branch
        or not github_token
        or not github_repo_owner
    ):
        raise ValueError(
            "Github environment variables not set. Please set them in your .env file."
        )
    ...
    existing_file = requests.get(
        url, headers=headers, params={"ref": github_repo_branch}
    )
    ...
    if response.status_code in [200, 201]:
        ...
        bt.logging.error(
            f"Failed to upload file for {filename}: {response.status_code} {response.text}"
        )
        return False
