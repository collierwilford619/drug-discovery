...
def get_sequence_from_protein_code(protein_code: str) -> str:
    ...
    if response.status_code == 200:
        ...
        if not amino_acid_sequence:
            bt.logging.warning(
                f"Retrieved empty sequence for {protein_code} from UniProt API"
            )
        else:
    ...
    try:
        ...
        for i in range(len(dataset)):
            if dataset[i]["Entry"] == protein_code:
                ...
                bt.logging.info(
                    f"Found sequence for {protein_code} in Hugging Face dataset"
                )
                return sequence

        bt.logging.error(
            f"Could not find protein {protein_code} in Hugging Face dataset"
        )
        return None
...
def get_challenge_params_from_blockhash(
    block_hash: str,
    weekly_target: str,
    num_antitargets: int,
    ...
    if not weekly_target or num_antitargets < 0:
        raise ValueError(
            "weekly_target must exist and num_antitargets must be non-negative."
        )
    ...
    result = {"targets": targets, "antitargets": antitargets}
    ...
    if include_reaction:
        try:
            ...
            bt.logging.warning(
                f"Failed to determine allowed reaction: {e}, defaulting to all reactions allowed"
            )
