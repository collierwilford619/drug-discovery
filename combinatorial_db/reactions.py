...
def get_molecules(mol_ids: list, db_path: str) -> list:
    try:
        ...
        for mol_id in mol_ids:
            cursor.execute(
                "SELECT smiles, role_mask FROM molecules WHERE mol_id = ?", (mol_id,)
            )
            result = cursor.fetchone()
...
def combine_triazole_synthons(azide_smiles: str, alkyne_smiles: str) -> str:
    ...
    try:
        ...
        a1 = next(
            (
                i
                for i, atom in enumerate(m1.GetAtoms())
...
def react_three_components(
    rxn_id: int, mol1_id: int, mol2_id: int, mol3_id: int, db_path: str
) -> str:
    try:
        reaction_info = get_reaction_info(rxn_id, db_path)
        ...
        if rxn_id == 5:  # suzuki_bromide_then_chloride (two-step cascade)
            ...
            intermediate = perform_smarts_reaction(
                reactant1, reactant2, suzuki_br_smarts
            )
            if not intermediate:
                return None

            # Then couple at chloride
            final_product = perform_smarts_reaction(
                intermediate, reactant3, suzuki_cl_smarts
            )
        ...
        bt.logging.error(
            f"Error in 3-component reaction {mol1_id}, {mol2_id}, {mol3_id}: {e}"
        )
        return None
...
def get_smiles_from_reaction(product_name):
    ...
    try:
        ...
        if len(parts) == 4:
            ...
            rxn_id, mol1_id, mol2_id, mol3_id = (
                int(rxn_id),
                int(mol1_id),
                int(mol2_id),
                int(mol3_id),
