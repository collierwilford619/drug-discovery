...
class AutoUpdater:
    ...
    def _check_for_updates(self):
        ...
        returncode, stdout, stderr = self._run_git_command(
            "diff", f"HEAD..origin/{self.BRANCH}"
        )
        if returncode != 0:
    ...
    async def start_update_loop(self):
        ...
        while True:
            try:
                self.logger.info(
                    f"Checking for updates from {self.REMOTE_URL} ({self.BRANCH} branch)"
                )
                ...
                if code_updates:
                    ...
                    if self._pull_updates():
                        self.logger.info(
                            "Code updates successfully applied, restarting"
                        )
    ...
    def _update_database(self):
        ...
        try:
            api_url = (
                "https://huggingface.co/api/datasets/Metanova/Mol-Rxn-DB/tree/main"
            )
