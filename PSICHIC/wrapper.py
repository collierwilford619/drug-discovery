...
class PsichicWrapper:
    def __init__(self):
        ...
        with open(
            os.path.join(self.runtime_config.MODEL_PATH, "config.json"), "r"
        ) as f:
            self.model_config = json.load(f)

    def load_model(self):
        degree_dict = torch.load(
            os.path.join(self.runtime_config.MODEL_PATH, "degree.pt"), weights_only=True
        )
        ...
        self.model.load_state_dict(
            torch.load(param_dict, map_location=self.device, weights_only=True)
        )
        self.model.eval()
    ...
    def initialize_protein(self, protein_seq: str) -> dict:
        ...
        sanitized_protein_seq = "".join(
            [aa if aa in allowed_chars else "X" for aa in protein_seq]
        )
    ...
    def create_screen_loader(self, protein_dict, smiles_dict):
        ...
        dataset = ProteinMoleculeDataset(
            self.screen_df, smiles_dict, protein_dict, device=self.device
        )
