...
def create_custom_sampler(class_list, specified_weight={}):
    ...
    class_weight = {
        t: 1.0 / len(np.where(class_list == t)[0]) for t in np.unique(class_list)
    }
...
def ligand_degree_from_dict(ligand_dict):
    ...
    for k, v in tqdm(ligand_dict.items()):
        ...
        clique_d = degree(
            clique_edge_index[1], num_nodes=clique_x.shape[0], dtype=torch.long
        )
        clique_max_degree = max(clique_max_degree, int(clique_d.max()))
...
def store_result(
    ...
    for idx, key in enumerate(interaction_keys):
        ...
        if mcls_pred is not None:
            if (
                "predicted_antagonist" in df.columns
                and "predicted_nonbinder" in df.columns
                and "predicted_agonist" in df.columns
            ):
                df.loc[
                    matching_row,
                    [
                        "predicted_antagonist",
                        "predicted_nonbinder",
                        "predicted_agonist",
                    ],
                ] = mcls_pred[idx].tolist()
            else:
        ...
        if all([idx in attention_dict["cluster_s"] for idx in range(3)]):
            unbatched_cluster_s0 = unbatch_nodes(
                attention_dict["cluster_s"][0].softmax(dim=-1),
                attention_dict["protein_residue_index"],
            )
            unbatched_cluster_s1 = unbatch_nodes(
                attention_dict["cluster_s"][1].softmax(dim=-1),
                attention_dict["protein_residue_index"],
            )
            unbatched_cluster_s2 = unbatch_nodes(
                attention_dict["cluster_s"][2].softmax(dim=-1),
                attention_dict["protein_residue_index"],
            )
        ...
        if save_interpret:
            for pair_id in df[matching_row]["ID"]:
                ...
                protein_interpret = pd.DataFrame(
                    {
                        ...
                        "PSICHIC_Residue_Score": minmax_norm(
                            unbatched_residue_score[idx].cpu().flatten().numpy()
                        ),
                    }
                )

                protein_interpret["Residue_ID"] = protein_interpret.index + 1
                protein_interpret["PSICHIC_Residue_Percentile"] = percentile_rank(
                    protein_interpret["PSICHIC_Residue_Score"]
                )
                protein_interpret = protein_interpret[
                    [
                        "Residue_ID",
                        "Residue_Type",
                        "PSICHIC_Residue_Score",
                        "PSICHIC_Residue_Percentile",
                    ]
                ]

                if all([id_ in attention_dict["cluster_s"] for id_ in range(3)]):
                    for ci in range(5):
                        protein_interpret["Layer0_Cluster" + str(ci)] = (
                            unbatched_cluster_s0[idx][:, ci].cpu().flatten().numpy()
                        )

                    for ci in range(10):
                        protein_interpret["Layer1_Cluster" + str(ci)] = (
                            unbatched_cluster_s1[idx][:, ci].cpu().flatten().numpy()
                        )

                    for ci in range(20):
                        protein_interpret["Layer2_Cluster" + str(ci)] = (
                ...
                if not successful_ligand:
                    print(
                        "Ligand Intepretation for {} failed due to not matching atom order.".format(
                            pair_id
                        )
                    )
                ## STORE Fingerprint
...
def virtual_screening(
    ...
    with torch.no_grad():
        for data in tqdm(data_loader):
            data = data.to(device)
            reg_pred, cls_pred, mcls_pred, sp_loss, o_loss, cl_loss, attention_dict = (
                model(
                    # Molecule
                    mol_x=data.mol_x,
