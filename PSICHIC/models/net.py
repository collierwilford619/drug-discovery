...
class net(torch.nn.Module):
        ...
        for idx in range(total_layer):
            ...
            self.prot_convs.append(
                Protein_PNAConv(
                    prot_deg,
                    hidden_channels,
                    edge_channels=hidden_channels,  # None,
                    pre_layers=pre_layers,
                    post_layers=post_layers,
                    aggregators=aggregators,
                    scalers=scalers,
            ...
            self.cluster.append(
                GCNCluster(
                    [hidden_channels, hidden_channels * 2, self.num_cluster[idx]],
                    in_norm=True,
                )
            )
    ...
    def forward(
        ...
        clique_x,
        clique_edge_index,
        atom2clique_index,  # drug cliques
        # Protein
        residue_x,
        residue_evo_x,
        residue_edge_index,
        ...
        spectral_loss = torch.tensor(0.0).to(self.device)
        ortho_loss = torch.tensor(0.0).to(self.device)
        ...
        for idx in range(self.total_layer):
            ...
            if self.drop_residue != 0 and self.training:
                _, _, residue_drop_mask = dropout_node(
                    ...
                    prot_batch,
                    self.training,
                )  # drop residue for regularization
    ...
    def freeze_backbone_optimizers(
    ...
    ):  ## only for fineTune Pretrain model
        """
        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                ...
                if pn.endswith("bias") or pn.endswith(
                    "mean_scale"
                ):  # or pn.endswith('logit_scale'):
        ...
        optimizer = torch.optim.AdamW(
            optim_groups, lr=learning_rate, betas=betas, eps=eps, amsgrad=amsgrad
        )
...
def compute_connectivity(
    edge_index, batch
):  ## for numerical stability (i.e. we cap inv_con at 100)
