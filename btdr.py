    ...
    def get(self, round_number: int, retry_if_too_early=False) -> str:
        ...
        while a <= self.RETRY_LIMIT:
            ...
            response: requests.Response = self.session.get(
                f"{self.url}/public/{round_number}"
            )
            if response.status_code == 200:
                break
            elif response.status_code in (404, 425):
                bt.logging.debug(
                    f"Randomness for round {round_number} is not yet available."
                )
                ...
                bt.logging.debug(
                    f"{response.status_code} {response} {response.headers} {response.text}"
                )
...
class AbstractBittensorDrandTimelock:
    ...
    def encrypt(self, uid: int, message: str, current_block: int) -> Tuple[int, bytes]:
        ...
        next_epoch_boundary = (
            (current_block // self.EPOCH_LENGTH) + 1
        ) * self.EPOCH_LENGTH
        ...
        target_round: int = self.get_current_round() + (
            (target_block - current_block) * 4
        )

        bt.logging.info(
            f"Encrypting message for UID {uid}... Unlockable at round {target_round} (block {target_block})"
        )
    ...
    def decrypt(
        ...
        if not plaintext.startswith(expected_prefix):
            bt.logging.warning(
                f"UID mismatch: Expected {expected_prefix} but got {plaintext}"
            )
            return None
    ...
    def decrypt_dict(
        self, encrypted_dict: Dict[int, Tuple[int, bytes]]
        ...
        for uid, (target_round, ciphertext) in encrypted_dict.items():
            try:
                ...
                decrypted_dict[uid] = self.decrypt(
                    uid, ciphertext, target_round, signature
                )
            except RuntimeError:
                current_round = self.get_current_round()
                bt.logging.warning(
                    f"Skipping UID {uid}: Too early to decrypt: {target_round=}, {current_round=}"
                )
...
async def async_decrypt_example(encrypted_dict, bdt) -> None:
    ...
    with ThreadPoolExecutor() as executor:
        try:
            decrypted_dict = await loop.run_in_executor(
                executor, bdt.decrypt_dict, encrypted_dict
            )
            logger.info(f"Decrypted Dictionary: {decrypted_dict}")
        except RuntimeError:
            logger.error("Decryption failed for one or more entries.")
